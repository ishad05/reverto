...
type DashboardTab = "listings" | "enquiries";
...
export function SellerDashboard({
  ...
  const revenueEst = useMemo(
    () => products.reduce((sum, p) => sum + (p.price_per_quantity ?? 0) * (p.quantity ?? 0), 0),
    [products],
  );
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <p className="text-xs text-gray-400 mt-1">
              {enquiries.filter((e) => e.status === "Open" || e.status === "Negotiating").length} active
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-600">Revenue (Est.)</span>
              <TrendingUp className="w-5 h-5 text-orange-400" />
            </div>
            {isLoading ? (
              <div className="h-8 w-20 bg-gray-100 rounded animate-pulse" />
            ) : (
              <p className="text-3xl font-bold text-gray-900">{fmtRevenue(revenueEst)}</p>
            )}
        ...
        <div className="flex gap-2 border-b border-gray-200">
          {(["listings", "enquiries"] as DashboardTab[]).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "listings" ? "My Listings" : "Enquiries"}
              {tab === "enquiries" && enquiries.filter((e) => e.status === "Open" || e.status === "Negotiating").length > 0 && (
                <span className="ml-1.5 inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-600 text-white text-xs font-bold">
                  {enquiries.filter((e) => e.status === "Open" || e.status === "Negotiating").length}
                </span>
              )}
            </button>
          ))}
